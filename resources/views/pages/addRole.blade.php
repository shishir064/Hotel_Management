...
@section('content')
    <form action="{{ route('store_role') }}" method="POST">
        ...
        <div>
            <h1 class="text-4xl md:text-[40px] outfit">Add Role</h1>
            <p class="text-sm md:text-base text-gray-500/90 mt-2 max-w-174">Fill in the details carefully.</p>
        </div>


        <div class="">
            <h2 class="text-gray-800 mt-10">Add Role</h2>
            <input type="text" placeholder="Add Role" name="name" value="{{ old('name') }}"
                class="py-2 px-2 rounded border border-gray-300 text-gray-500 max-w-42">
        </div>
        <div class="mb-8">
            <button class="bg-primary text-white px-8 py-2 rounded mt-8 cursor-pointer">Add Role</button>
        </div>
    </form>
